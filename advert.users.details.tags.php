...
if (cot_auth('advert', 'any', 'W') || cot::$usr["isadmin"]){
    $submitNewAdvUrl = cot_url('advert', array('a'=>'edit'));
    $submitNewAdv = cot_rc_link($submitNewAdvUrl, cot::$L['advert_add_new']);
}

$advUnValidated = ($urr['user_id'] == cot::$usr['id'] || cot::$usr["isadmin"] || cot_auth('advert', 'any', 'A'));
...
if (!$advUnValidated){
    ...
    $advCond[] = array('state', advert_model_Advert::PUBLISHED);
}
...
$t->assign(array(
    ...
    "USERS_DETAILS_ADVERT_COUNT" => advert_model_Advert::count($advCond),
    "USERS_DETAILS_ADVERT_URL" => cot_url('advert', array('m' => 'user', 'uid' => $urr['user_id'])),
